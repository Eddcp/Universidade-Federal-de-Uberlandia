...
int main()
{
  ...
  //  float mediaFitIndividuosGeral=0.0, mediaFitMenoresCem=0.0;
    ...
    strcpy(palavra1, "cross");
    strcpy(palavra2, "roads");
    strcpy(palavra3, "danger");
...
individuo* avaliaPopulacaoGenerico(individuo individuos[], int qtdPopulacao, int qtdFilhos, int tamanhoIndividuo)
{
  ...
  /*printf("Antes\n");
  printf("String 1 eh: %d\n", strInt1);
  printf("String 2 eh: %d\n", strInt2);
  printf("String 3 eh: %d\n", strInt3);

  strInt1 = individuos[0].genes[0]*1000 + individuos[0].genes[1]*100 + individuos[0].genes[2]*10 + individuos[0].genes[3];
  strInt2 = individuos[0].genes[4]*1000 + individuos[0].genes[5]*100 + individuos[0].genes[6]*10 + individuos[0].genes[1];
  strInt3 = individuos[0].genes[4]*10000 + individuos[0].genes[5]*1000 + individuos[0].genes[2]*100 + individuos[0].genes[1]*10 + individuos[0].genes[7];
 ...
 // getchar();
  for(k=0; k<(qtdPopulacao+qtdFilhos); k++)
  {
      //fitness = abs(MONEY-(SEND + MORE))
        //Primeira string sempre está no início da string resultante, por exemplo send está em sendmory, eat está em eathpl,...
      j= strlen(palavra1)-1;
      ...
      {
         strInt1 *= 10;
         strInt1 += individuos[k].genes[resultante[palavra1[i] - 'a']];
      }
      ...
      {
         strInt2 *= 10;
         strInt2 += individuos[k].genes[resultante[palavra2[i] - 'a']];
      }
      ...
      {
         strInt3 *= 10;
         strInt3 += individuos[k].genes[resultante[palavra3[i] - 'a']];
      }

      //Faz calculo do fitness
      individuos[k].avaliacao = abs(strInt3 - (strInt1 + strInt2));
      strInt1 = 0;
      strInt2 = 0;
      strInt3 = 0;
...
individuo* novaGeracaoTorneio(individuo individuos[], int qtdPopulacaoInicial, int qtdFilhos, int tamanhoIndividuo)
{
  ...
  //    printf("Atribuindo individuo %d para individuo %d\n", posicaoMelhorTorneio,(qtdPopulacaoInicial+pais));
    ...
    for(i=qtdPopulacaoInicial; i<(qtdPopulacaoInicial+qtdFilhos)-1; i++)
    {
        ...
        printf("Pai %d: \n", i);
        for(k=0; k<tamanhoIndividuo; k++)
        {
          printf("%d ", individuos[i].genes[k]);
        ...
        for(k=0; k<tamanhoIndividuo; k++)
        {
          printf("%d ", individuos[i+1].genes[k]);
...
individuo* novaGeracaoRoleta(individuo individuos[], int qtdPopulacaoInicial, int qtdFilhos, int tamanhoIndividuo)
{
    ...
    if(!strcmp(palavra1, "cross") || !strcmp(palavra1, "donald"))
    {
      inverteRoleta = 1000000;
    }
    else
      inverteRoleta = 100000;
...
individuo* pmx(individuo pai1, individuo pai2,int tamanhoIndividuo)
{
    ...
    printf("primeiro ponto de corte: %d", comeco);
    printf("segundo ponto de corte: %d", fim);
